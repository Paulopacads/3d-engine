...
int main(int, char **)
{
    ...
    std::shared_ptr<Texture> color =
        std::make_shared<Texture>(window_size, ImageFormat::RGBA8_UNORM);
    std::shared_ptr<Texture> normal =
        std::make_shared<Texture>(window_size, ImageFormat::RGBA8_UNORM);
    std::shared_ptr<Texture> depth =
        std::make_shared<Texture>(window_size, ImageFormat::Depth32_FLOAT);
    Framebuffer gbuffer(depth.get(), std::array{ color.get(), normal.get() });

    std::shared_ptr<Texture> lit =
        std::make_shared<Texture>(window_size, ImageFormat::RGBA16_FLOAT);
    Framebuffer main_framebuffer(depth.get(), std::array{ lit.get() });

    std::shared_ptr<Texture> tonemap_color =
        std::make_shared<Texture>(window_size, ImageFormat::RGBA8_UNORM);
    ...
    auto bloom_down =
        Program::from_files("downsampling.frag", "downsampling.vert");
    auto bloom_up = Program::from_files("upsampling.frag", "upsampling.vert");
